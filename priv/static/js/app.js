...
require.register("web/elm/TheJuice.elm", function(exports, require, module) {
...
var elmDiv = document.getElementById('elm-main'),
    elmApp = Elm.TheJuice.embed(elmDiv);
...
var userToken = (0, _jquery2.default)("meta[name='channel_token']").attr("content");
...
var CREATED_COMMENT = "CREATED_COMMENT";
var APPROVED_COMMENT = "APPROVED_COMMENT";
var DELETED_COMMENT = "DELETED_COMMENT";

// Grab the current post's id from a hidden input on the page
var postId = (0, _jquery2.default)("#post-id").val();
...
// Based on a payload, return to us an HTML template for a comment
// Consider this a poor version of JSX
var createComment = function createComment(payload) {
...
var getCommentAuthor = function getCommentAuthor() {
  return (0, _jquery2.default)("#comment_author").val();
};
// Provide the comment's body from the form
var getCommentBody = function getCommentBody() {
  return (0, _jquery2.default)("#comment_body").val();
};
// Based on something being clicked, find the parent comment id
var getTargetCommentId = function getTargetCommentId(target) {
  return (0, _jquery2.default)(target).parents(".comment").data("comment-id");
};
// Reset the input fields to blank
var resetFields = function resetFields() {
  (0, _jquery2.default)("#comment_author").val("");
  (0, _jquery2.default)("#comment_body").val("");
};

// Push the CREATED_COMMENT event to the socket with the appropriate author/body
(0, _jquery2.default)(".create-comment").on("click", function (event) {
  event.preventDefault();
  channel.push(CREATED_COMMENT, { author: getCommentAuthor(), body: getCommentBody(), postId: postId });
  resetFields();
});

// Push the APPROVED_COMMENT event to the socket with the appropriate author/body/comment id
(0, _jquery2.default)(".comments").on("click", ".approve", function (event) {
  event.preventDefault();
  var commentId = getTargetCommentId(event.currentTarget);
  // Pull the approved comment author
  var author = (0, _jquery2.default)("#comment-" + commentId + " .comment-author").text().trim();
  // Pull the approved comment body
  var body = (0, _jquery2.default)("#comment-" + commentId + " .comment-body").text().trim();
  channel.push(APPROVED_COMMENT, { author: author, body: body, commentId: commentId, postId: postId });
});

// Push the DELETED_COMMENT event to the socket but only pass the comment id (that's all we need)
(0, _jquery2.default)(".comments").on("click", ".delete", function (event) {
  event.preventDefault();
  var commentId = getTargetCommentId(event.currentTarget);
  channel.push(DELETED_COMMENT, { commentId: commentId, postId: postId });
});

// Handle receiving the CREATED_COMMENT event
channel.on(CREATED_COMMENT, function (payload) {
  ...
  (0, _jquery2.default)(".comments h2").after(createComment(payload));
});

// Handle receiving the APPROVED_COMMENT event
channel.on(APPROVED_COMMENT, function (payload) {
  // If we don't already have the right comment, then add it to the DOM
  if ((0, _jquery2.default)("#comment-" + payload.commentId).length === 0) {
    (0, _jquery2.default)(".comments h2").after(createComment(payload));
  }
  // And then remove the "Approve" button since we know it has been approved
  (0, _jquery2.default)("#comment-" + payload.commentId + " .approve").remove();
});

// Handle receiving the DELETED_COMMENT event
channel.on(DELETED_COMMENT, function (payload) {
  // Just delete the comment from the DOM
  (0, _jquery2.default)("#comment-" + payload.commentId).remove();
});
...
var _user$project$TheJuice$main = _elm_lang$virtual_dom$Native_VirtualDom.staticProgram(
	_elm_lang$html$Html$text('Hello from Elm'));

var Elm = {};
Elm['TheJuice'] = Elm['TheJuice'] || {};
if (typeof _user$project$TheJuice$main !== 'undefined') {
    _user$project$TheJuice$main(Elm['TheJuice'], 'TheJuice', undefined);
}
